...
async function main() {
  ...
  let buffer_below = roundToNearestMinute(new Date().addHours(24));
  buffer_below.subtractMinutes(5);
  let buffer_above = roundToNearestMinute(new Date().addHours(24));
  ...
  for (let i = 0, y = 0; i < messages.length; i += chunkSize, y++) {
    ...
    p_mail[y] = createPromiseFactory(async () => {
      await sendMassEmailMailjet(
        chunk,
        template_ids.machine_twenty_four,
        true,
        "MACHINE_TWENTY_FOUR"
      );
    }, 200);
